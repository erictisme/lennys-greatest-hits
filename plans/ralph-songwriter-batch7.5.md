# Ralph Songwriter Loop — Batch 7.5
...
This is a long-running loop. Work through the tasks in `content/songs/batch7.5/tasks.json` one at a time. For each song, execute Phase 1 (Research) then Phase 2 (Songwriting). Save all outputs to the batch7.5 folder.

**BATCH 7.5 CONTEXT:** This is the FINAL EXPANSION — 9 songs closing every remaining gap in the catalog. After this batch, the catalog is complete at 104 tracks. These songs fill thin albums (The Talent Search, By the Numbers) and add iconic founders (Drew Houston, Ben Horowitz, Shopify) plus AI builders (Boris Cherny/Claude Code, Nick Turley/ChatGPT). Every song must be a standout.
...
6. `content/songs/batch7.5/feedback.md` — Accumulated rules from all previous batches + per-song creative direction
7. `content/lyrics/04-taste.md` — Best lyric example (quality reference)

---

## The Loop

For each incomplete task in `content/songs/batch7.5/tasks.json`:

### Phase 1: Research

1. **Read the source material** using Lenny MCP:
   - `mcp__lennysdata__read_content` with the filename from the task's `source_files`
...
6. **Save research brief** to `content/songs/batch7.5/research/{output_file}.md`
...
3. **Write the first draft:**
   ...
   - Follow ALL rules from lyric-craft-guide.md:
     ...
     - Chorus repeats IDENTICALLY (only final chorus may vary 1 line)
...
4. **Critique the draft (Rick Rubin Mode):**
   ...
   - The Chorus Test: does it repeat identically? (Only final chorus may vary 1 line)
...
7. **Create Suno prompt** with a UNIQUE SONIC SIGNATURE for this song (save to `content/songs/batch7.5/suno-prompts/{slug}.txt`):

```
TITLE: [Song Title]

STYLE: [Detailed — reference artists from artist-palette.md, specific instruments, vocal character, BPM 72-120, key (use varied keys: F#, Bb, Ab, Db, Gb, B — not just G/C/D), mood. Include a UNIQUE sonic element that defines this song (e.g., cello-driven, muted trumpet, finger snaps as rhythm, ticking clock, pizzicato strings). Very specific.]
...
8. **Save lyrics + metadata** to `content/songs/batch7.5/lyrics/{output_file}.md`
...
9. **Run final quality checklist:**
   ...
   - [ ] Chorus repeats identically (final chorus may vary 1 line)
   - [ ] No stat-stuffing
   - [ ] End-stopped lines
   ...
   - [ ] Key is not G, C, or D (use varied keys)

10. **Mark task complete** in tasks.json with timestamp.

### After Each Song

Log a one-line summary: `[Song Title] — [genre] — [key] — [killer line] — [any concerns]`

If you notice the batch is getting sonically monotonous (3+ songs in same genre/key), actively push the next song in a different direction.
...
- **Chorus repeats identically.** Only the FINAL chorus may vary 1 line for emotional payoff.
- **Unique sonic signature per song.** Every song needs ONE defining instrument/texture.
- **Varied keys.** Use F#, Bb, Ab, Db, Gb, B — NOT just G/C/D.
- **When in doubt, strip it back.** Rick Rubin: the simplest, most honest version wins.
- **9 songs means every one must be a STANDOUT.** No coasting, no filler. This batch closes the catalog for good.
- **Check for Stripe/AI overlap.** Stripe already has 3 songs, Electric Dreams has 6. Find fresh angles — focus on IDEAS and PEOPLE, not companies and tools.
