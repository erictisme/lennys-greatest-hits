...
This is a long-running loop. Work through the tasks in `content/songs/batch5/tasks.json` one at a time. For each song, execute Phase 1 (Research) then Phase 2 (Songwriting). Save all outputs to the batch5 folder.

**BATCH 5 CONTEXT:** This is the FINAL BATCH — 20 songs filling the last iconic gaps. PLG, strategy, Superhuman, Radical Candor, OKRs, onboarding, presentations, marketplace, category design, resilience. After this batch we'll have 87 songs total. The bar is the highest it's ever been.
...
6. `content/songs/batch5/feedback.md` — Accumulated rules from Batches 1-4
...
For each incomplete task in `content/songs/batch5/tasks.json`:
...
6. **Save research brief** to `content/songs/batch5/research/{output_file}.md`
...
7. **Create Suno prompt** with a UNIQUE SONIC SIGNATURE for this song (save to `content/songs/batch5/suno-prompts/{slug}.txt`):
...
8. **Save lyrics + metadata** to `content/songs/batch5/lyrics/{output_file}.md`
...
- **20 songs means VARIETY IS EVERYTHING.** Plan genre spread. No two consecutive songs in the same genre.
