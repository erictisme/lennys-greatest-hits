...
This is a long-running loop. Work through the tasks in `content/songs/batch4/tasks.json` one at a time. For each song, execute Phase 1 (Research) then Phase 2 (Songwriting). Save all outputs to the batch4 folder.

**BATCH 4 CONTEXT:** These are ICONIC Lenny episodes that have no song yet. Pricing, strategy, storytelling, culture, sales, GTM. The bar is higher — these topics are foundational and Lenny's audience will expect them to be represented well.
...
6. `content/songs/batch4/feedback.md` — Accumulated rules from Batches 1-3
...
For each incomplete task in `content/songs/batch4/tasks.json`:
...
6. **Save research brief** to `content/songs/batch4/research/{output_file}.md`
...
7. **Create Suno prompt** with a UNIQUE SONIC SIGNATURE for this song (save to `content/songs/batch4/suno-prompts/{slug}.txt`):
...
8. **Save lyrics + metadata** to `content/songs/batch4/lyrics/{output_file}.md`
...
- **Chorus repeats identically.** Only the FINAL chorus may vary 1 line for emotional payoff. This is the biggest change from batch 3.
